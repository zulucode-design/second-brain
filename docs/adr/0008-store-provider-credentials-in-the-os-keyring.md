...
a later retry. Settings writes also update the credential store before the redacted config and
roll back the first write if the second fails. This fail-closed boundary avoids both plaintext
fallback and credential loss, while still allowing a headless process without a credential
store to start with provider features unconfigured.
