...
//!
//! ADR-0002's shared settings file now exists at `.helixnotes/settings.json` and initially
//! owns snapshot retention. [`DatabaseRegistry`] remains in its focused file beside the
//! note map: it is structured integration identity rather than a user setting. Both files
//! are vault-scoped and credential-free, which preserves the correctness boundary without
//! coupling the Notion adapter to the general settings schema.
//!
